...
- **`ia` ↔ artifact sync check** — `wireframe/tools/check-ia-sync.py`: asserts the handful of facts this corpus and the built artifact both state (taxonomy lists, nav slot count, heading link/label claims, declared-`full` pages existing, heading names the doc quotes actually existing). Non-zero exit = drift. Run it after any structural change to either side, and before editing `ia` at scale. The instrument exists because the same class of drift recurred twice: the hygiene answer to a recurring failure is a mechanical check, not another prose rule. It carries its own negative tests in its docstring's spirit — a check never shown to fail is a hope. When the front-end repo takes over the corpus, this check is re-pointed at the production components, not retired. It also asserts that `ia`'s table of contents is current; the TOC is generated by `wireframe/tools/gen-ia-toc.py` and never hand-edited, so a renamed heading fails the check until it is regenerated. That generator's anchor-slug rule was verified against GitHub's own markdown renderer rather than assumed — re-run that comparison before changing it.
- **Link resolver** — `wireframe/tools/check-links.py`: resolves every relative `href`/`src` in every prototype page against the filesystem, and every cross-page `#anchor` against the target's real ids. The nav and footer are pasted into every page, so one route rename moves hundreds of links and a wrong relative prefix 404s silently while the page still renders — nothing else catches that. `--baseline` records the current state, `--compare` fails only on regressions. Run it before and after any route move.

...
| `fig-conv` | `front-end/figma-conventions.md` | living | the Figma sketchpad: the file's state, how its variables resolve, and the handoff read-out contract — which groups become CSS and which are ignored | — |
| `fig-map` | `front-end/figma-css-map.md` | living | where the kit's Figma variables and shadcn's CSS diverge and how each class translates: mirrors, Figma mechanics, kit extras, internals; our own layers (`vecto_colors`, Tailwind additions, `VECTO Responsive`); the Luma recipe deviations; the radius and colour-space traps | `fig-conv` §6 for what to read; `f2c` for code-side rules |
| `brand` | `front-end/brand-layer.md` | living | what VECTO changed or added on top of shadcn Luma — each customization in brief: what it is, which convention it extends, where it lives | `fig-map` for translation; `fig-conv` for how the file resolves |
| `f2c` | `front-end/figma-to-code.md` | living | code-side translation: component-property → cva mapping, text/effect styles, theme modes & the inverted-section contract, typeface, the content model's prop-type expression, the verbatim-naming contract, `/styleguide` | `fig-conv` for the read-out contract |
...
| `tokens` | `front-end/tokens.md` *(planned — created when the token spec is authored, build Phase 1; seeds: `token-variant-spec-draft.md` and `font-strategy.md` in the repo-root `explorations/` folder)* | living | the VECTO token spec: branded ramps, semantic utilities, the type roles and their stepped-or-fluid scaling | `fig-conv` for the read-out contract |
...
| `skills` | `skills-routing.md` | living | the skill-usage ritual (declare session type → load → name in report), the research-before-design pass, how its findings carry into a bespoke build, and its recency/scale-fit checks, the Cowork/Code routing maps, subject-matter override, design-skill routing | `fe-arch` §4 for the scaffold-time encoding, §4a for Impeccable's context files |
